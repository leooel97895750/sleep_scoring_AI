...
total = double(ini_data(:,[3 4 8 9]));
eog_total = total(:,3);
emg_total = total(:,4);

myf3 = total(:, 1); % 因為跟後面fft變數重複
f4 = total(:, 2);
...
for epoch_seq=1 : floor(L_total/epoch) 
    if epoch_seq == 1   %head
        ...
        epoch_f3 = myf3(1 : epoch_seq*epoch + epoch_overlapping);
        epoch_f4 = f4(1 : epoch_seq*epoch + epoch_overlapping);
    elseif epoch_seq == floor(L_total/epoch)   %tail
        ...
        epoch_f3 = myf3(1+(epoch_seq-1)*epoch - epoch_overlapping : epoch_seq*epoch);
        epoch_f4 = f4(1+(epoch_seq-1)*epoch - epoch_overlapping : epoch_seq*epoch);
    else
        ...
        epoch_f3 = myf3(1+(epoch_seq-1)*epoch - epoch_overlapping : epoch_seq*epoch + epoch_overlapping);
        epoch_f4 = f4(1+(epoch_seq-1)*epoch - epoch_overlapping : epoch_seq*epoch + epoch_overlapping);
    end
