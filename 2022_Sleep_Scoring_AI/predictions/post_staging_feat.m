...
function post_result = post_staging_feat(auto_staging,feat)
    ...
    for epoch_no = 1:length(auto_staging_ini)-3
        ...
        if epoch_no > 1
            if isempty(find(auto_staging_ini(1:epoch_no-1) == 1 | auto_staging_ini(1:epoch_no-1) == 2, 1)) == 1 && auto_staging_ini(epoch_no) == 3
                auto_staging_ini(epoch_no) = 0;
            end
            if isempty(find(auto_staging_ini(1:epoch_no-1) == 2 | auto_staging_ini(1:epoch_no-1) == 3, 1)) == 1 && auto_staging_ini(epoch_no) == -1
