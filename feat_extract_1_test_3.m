function ret_result = feat_extract_1_test_3(ini_data,fs,ns)
    
    if size((ini_data),2) == 3
        total = double(ini_data(:,[1 2 3]));

        %3-channel (new 1299):C3M2,EOG,EMG
        eog_total = total(:,2);
        emg_total = total(:,3);
        eeg_c3_total = total(:,1);
    elseif size((ini_data),2) == 4
        total = double(ini_data(:,[1 2 3 4]));

        %4-channel (new PSG):C3,CZ,EOG,EMG
        eog_total = total(:,3);
        emg_total = total(:,4);
        eeg_c3_total = total(:,1);      
    elseif size((ini_data),2) == 5
        total = double(ini_data(:,[1 2 3 4 5]));
