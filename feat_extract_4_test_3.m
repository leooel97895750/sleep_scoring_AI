function feat_result = feat_extract_4_test_3(test_data,fs,ns)

%5-channel:C3,F4,E1,E2,EMG
data = double(test_data(:,3)); % EOG Channel
...
%%  extract feature
    ...
    [x1,y1] = butter(8, 12/100,'low');      % filter for alpha and beta
    [x2,y2] = butter(8, 8/100,'high');
    [x3,y3] = butter(8,18/(fs/2),'high');
    
    [a1,b1] = butter(8,15/100,'low');       % filter for spindle
    [a2,b2] = butter(8,12/100,'high');
    
    [i1,j1] = butter(3,2/100,'low');        % filter for delta
...
%         coeff_alpha = 0.5;
    ...
    feat_test(:,4) = TEO_STFT_PWR(data,fs,ns); % E1
