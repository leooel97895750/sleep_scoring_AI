clear 
%close all
data_files=dir('D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\data_mat_0111\*.mat');
stage_files=dir('D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\stage_dat_0111\*.dat');
...
auto_files=dir('D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\cut_channel_2EEG_2EOG_EMG_mat_chun\autostage_withsmoothing\*.dat');
auto_files2=dir('D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\cut_channel_2EEG_2EOG_EMG_mat\autostage_withsmoothing\*.dat');
% %%
...
%data=data.data;
autostage_chun=load([auto_files(data_num).folder '\' auto_files(data_num).name]);
autostage=load([auto_files2(data_num).folder '\' auto_files2(data_num).name]);
...
% save(['D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\feat_chun\' data_files(data_num).name(1:end-4) '_feat.mat'],'feat');
% feat = run_feat(data',fs);
% save(['D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\feat_o\' data_files(data_num).name(1:end-4) '_feat.mat'],'feat');

% [autostage nodestage] = run_purposed(data',feat);
% csvwrite(['D:\孟純\mandywu\睡眠判讀\data\睡眠data\子計畫五\autostage_o\' data_files(data_num).name(1:end-4) '_autostage.dat'],autostage);
...
autostage(find(autostage==4))=-1;autostage_chun(find(autostage_chun==4))=-1;
figure('outerposition',get(0,'screensize'));
subplot(312)
plot(autostage*-1,'r');hold on;
...
% end
subplot(313)
plot(autostage_chun*-1,'b');hold on;
% for i=1:length(low_reliability)
%     second_block = patch([low_reliability(i),low_reliability(i)+1,low_reliability(i)+1,low_reliability(i)], [1 1 -3 -3], 'b');
%     second_block.FaceAlpha = 0.2;second_block.EdgeAlpha = 0.2;
% hold on; 
